  ...
  useEffect(() => {
    async function load() {
      ...
      if (p?.role === 'manager') {
        ...
        // Direct assignments + assignments from handover history
        ...
        const allIds = [...new Set([...directIds, ...handoverAssignmentIds])]
        // Fetch any handover assignments not already in direct
        const missing = allIds.filter(id => !directIds.includes(id))
        ...
        // Only see assignments where they have tasks
        const { data: myTasks } = await supabase
          .from('tasks')
          .select('assignment_id')
          .eq('assigned_to', user.id)
        assignmentIds = [...new Set((myTasks ?? []).map(t => t.assignment_id as string))]
    ...
    if (!isOpen && !tasksByAssignment[assignmentId]) {
      ...
      if (taskIds.length > 0) {
        const { data: obsData } = await supabase
          .from('observations').select('*').in('task_id', taskIds).order('created_at')
                                    ...
                                    <div key={tId} className="bg-white rounded-lg border border-gray-100 overflow-hidden">
                                      <div className="flex items-center justify-between px-4 py-2.5">
                                        ...
                                        <div className="flex items-center gap-4 text-xs text-gray-500">
                                          ...
                                          }`}>{(t.status as string).replace(/_/g, ' ')}</span>
                                        </div>
                                      </div>
                                      {taskObs.length > 0 && (
